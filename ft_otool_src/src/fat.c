...
/*   Updated: 2018/11/08 16:13:41 by nhaquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	void *endptr)
{
	...
	while (nfat_arch--)
	{
		if (((void*)farch + sizeof(struct fat_arch_64)) > endptr)
			return (-2);
	...
	void *endptr)
{
	...
	while (nfat_arch--)
	{
		if (((void*)farch + sizeof(struct fat_arch)) > endptr)
			return (-2);
...
int		handle_fat(void *ptr, void *endptr, bool is_64)
{
	...
	if (((void*)header + sizeof(struct fat_header)) > endptr)
		return (-2);
	if (is_64)
		return(handle_arch_64(endian_4(header->nfat_arch), sizeof(struct fat_header), \
			ptr, endptr));
	else
		return(handle_arch_32(endian_4(header->nfat_arch), sizeof(struct fat_header), \
			ptr, endptr));
	return (0);
}
